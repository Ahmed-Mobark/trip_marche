...
import '../cubit/sponsored_trips_items_cubit.dart';
import '../cubit/sponsored_trips_items_state.dart';
import '../widgets/popular_trip_grid_card.dart';

/// Sponsored trips from [AppEndpoints.homeSponsoredTripsItems] — opened via Home "See all".
class SponsoredTripsListView extends StatelessWidget {
  const SponsoredTripsListView({super.key});

  @override
  Widget build(BuildContext context) {
    return BlocProvider(
      create: (_) => sl<SponsoredTripsItemsCubit>()..loadInitial(),
      child: const _SponsoredTripsListScaffold(),
    );
  }
}

class _SponsoredTripsListScaffold extends StatefulWidget {
  const _SponsoredTripsListScaffold();

  @override
  State<_SponsoredTripsListScaffold> createState() =>
      _SponsoredTripsListScaffoldState();
}

class _SponsoredTripsListScaffoldState extends State<_SponsoredTripsListScaffold> {
  ...
  void _applySearchFromField() {
    ...
    context.read<SponsoredTripsItemsCubit>().flushSearchNow(_searchCtrl.text);
  }
  ...
  void _onSearchClear() {
    ...
    context.read<SponsoredTripsItemsCubit>().flushSearchNow('');
  }
  ...
  void _onScroll() {
    ...
    if (pos.pixels >= pos.maxScrollExtent - 280) {
      context.read<SponsoredTripsItemsCubit>().loadMore();
    }
  }

  String _titleFromState(SponsoredTripsItemsState state) {
    final fromApi = state.meta?.sectionTitle?.trim() ?? '';
    if (fromApi.isNotEmpty) {
      return fromApi;
    }
    return context.tr.homeSponsoredTrips;
  }
  ...
  TripModel? _tripById(SponsoredTripsItemsState state, int tripId) {
  ...
  Widget build(BuildContext context) {
    return MultiBlocListener(
      listeners: [
        BlocListener<SponsoredTripsItemsCubit, SponsoredTripsItemsState>(
          listenWhen: (p, n) =>
          ...
          listener: (context, state) {
            ...
            context.read<SponsoredTripsItemsCubit>().clearWishlistError();
          },
        ),
      ],
      child: BlocBuilder<SponsoredTripsItemsCubit, SponsoredTripsItemsState>(
        builder: (context, state) {
          ...
          return Scaffold(
            ...
            body: RefreshIndicator(
              ...
              onRefresh: () =>
                  context.read<SponsoredTripsItemsCubit>().refresh(),
  ...
  Widget _buildBody(BuildContext context, SponsoredTripsItemsState state) {
    if (state.status == SponsoredTripsItemsStatus.initial ||
        (state.status == SponsoredTripsItemsStatus.loading &&
            state.trips.isEmpty)) {
      return ListView(
    ...
    if (state.status == SponsoredTripsItemsStatus.failure &&
        state.trips.isEmpty) {
      return ListView(
        physics: const AlwaysScrollableScrollPhysics(),
        ...
        children: [
          ...
          FilledButton(
            onPressed: () =>
                context.read<SponsoredTripsItemsCubit>().loadInitial(),
    ...
    final itemCount = state.trips.length +
        (state.status == SponsoredTripsItemsStatus.loadingMore ? 1 : 0);
                ...
                return PopularTripGridCard(
                  ...
                  onFavoriteTap: () async {
                    ...
                    final updated = _tripById(
                      context.read<SponsoredTripsItemsCubit>().state,
                      trip.id,
                    );
                    if (updated != null) {
                      _syncWishlistToParentShell(
