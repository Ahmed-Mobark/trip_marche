...
import '../cubit/popular_trips_items_cubit.dart';
import '../cubit/popular_trips_items_state.dart';
import '../cubit/special_trips_cubit.dart';
import '../widgets/popular_trip_grid_card.dart';

/// Popular trips from [AppEndpoints.homePopularTripsItems] — opened via Home "See all".
class PopularTripsListView extends StatelessWidget {
  const PopularTripsListView({super.key});

  @override
  Widget build(BuildContext context) {
    return BlocProvider(
      create: (_) => sl<PopularTripsItemsCubit>()..loadInitial(),
      child: const _PopularTripsListScaffold(),
    );
  }
}

class _PopularTripsListScaffold extends StatefulWidget {
  const _PopularTripsListScaffold();

  @override
  State<_PopularTripsListScaffold> createState() =>
      _PopularTripsListScaffoldState();
}

class _PopularTripsListScaffoldState extends State<_PopularTripsListScaffold> {
  ...
  void _applySearchFromField() {
    ...
    context.read<PopularTripsItemsCubit>().flushSearchNow(_searchCtrl.text);
  }
  ...
  void _onSearchClear() {
    ...
    context.read<PopularTripsItemsCubit>().flushSearchNow('');
  }
  ...
  void _onScroll() {
    ...
    if (pos.pixels >= pos.maxScrollExtent - 280) {
      context.read<PopularTripsItemsCubit>().loadMore();
    }
  }

  String _titleFromState(PopularTripsItemsState state) {
  ...
  TripModel? _tripById(PopularTripsItemsState state, int tripId) {
  ...
  Widget build(BuildContext context) {
    return MultiBlocListener(
      listeners: [
        BlocListener<PopularTripsItemsCubit, PopularTripsItemsState>(
          listenWhen: (p, n) =>
          ...
          listener: (context, state) {
            ...
            context.read<PopularTripsItemsCubit>().clearWishlistError();
          },
        ),
      ],
      child: BlocBuilder<PopularTripsItemsCubit, PopularTripsItemsState>(
  ...
  Widget _buildBody(BuildContext context, PopularTripsItemsState state) {
    if (state.status == PopularTripsItemsStatus.initial ||
        (state.status == PopularTripsItemsStatus.loading &&
            state.trips.isEmpty)) {
      return ListView(
    ...
    if (state.status == PopularTripsItemsStatus.failure &&
        state.trips.isEmpty) {
      return ListView(
        physics: const AlwaysScrollableScrollPhysics(),
        ...
        children: [
          ...
          FilledButton(
            onPressed: () =>
                context.read<PopularTripsItemsCubit>().loadInitial(),
                ...
                return PopularTripGridCard(
                  ...
                  onFavoriteTap: () async {
                    ...
                    final updated = _tripById(
                      context.read<PopularTripsItemsCubit>().state,
                      trip.id,
                    );
                    if (updated != null) {
                      _syncWishlistToParentShell(
