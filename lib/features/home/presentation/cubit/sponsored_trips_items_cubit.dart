...
import 'sponsored_trips_items_state.dart';
...
class SponsoredTripsItemsCubit extends Cubit<SponsoredTripsItemsState> {
  SponsoredTripsItemsCubit(this._homeRepository, this._wishlistRepository)
      : super(const SponsoredTripsItemsState());
  ...
  Future<void> loadInitial() async {
    emit(
      state.copyWith(
        status: SponsoredTripsItemsStatus.loading,
        trips: const [],
      ),
    );

    final result = await _homeRepository.getSponsoredTripsItems(
      page: 1,
    ...
    result.fold(
      (failure) {
        emit(
          state.copyWith(
            status: SponsoredTripsItemsStatus.failure,
            errorMessage: failure.message,
          ),
        );
      },
      (data) {
        final response = SponsoredTripsItemsResponse.fromJson(data);
        emit(
          state.copyWith(
            status: SponsoredTripsItemsStatus.success,
  ...
  Future<void> refresh() async {
    if (state.status != SponsoredTripsItemsStatus.success) {
      await loadInitial();
      return;
    }
  ...
  Future<void> loadMore() async {
    if (!state.hasMore ||
        state.status == SponsoredTripsItemsStatus.loadingMore ||
        state.status == SponsoredTripsItemsStatus.loading ||
        state.status != SponsoredTripsItemsStatus.success) {
      return;
    }

    emit(state.copyWith(status: SponsoredTripsItemsStatus.loadingMore));
    ...
    result.fold(
      (_) {
        emit(state.copyWith(status: SponsoredTripsItemsStatus.success));
      },
      (data) {
        final response = SponsoredTripsItemsResponse.fromJson(data);
        emit(
