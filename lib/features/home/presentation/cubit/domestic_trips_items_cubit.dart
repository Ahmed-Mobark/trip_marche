...
import 'domestic_trips_items_state.dart';
...
class DomesticTripsItemsCubit extends Cubit<DomesticTripsItemsState> {
  DomesticTripsItemsCubit(this._homeRepository, this._wishlistRepository)
      : super(const DomesticTripsItemsState());
  ...
  Future<void> loadInitial() async {
    emit(
      state.copyWith(
        status: DomesticTripsItemsStatus.loading,
        trips: const [],
      ),
    );

    final result = await _homeRepository.getDomesticTripsItems(
      page: 1,
    ...
    result.fold(
      (failure) {
        emit(
          state.copyWith(
            status: DomesticTripsItemsStatus.failure,
            errorMessage: failure.message,
          ),
        );
      },
      (data) {
        final response = DomesticTripsItemsResponse.fromJson(data);
        emit(
          state.copyWith(
            status: DomesticTripsItemsStatus.success,
  ...
  Future<void> refresh() async {
    if (state.status != DomesticTripsItemsStatus.success) {
      await loadInitial();
      return;
    }
  ...
  Future<void> loadMore() async {
    if (!state.hasMore ||
        state.status == DomesticTripsItemsStatus.loadingMore ||
        state.status == DomesticTripsItemsStatus.loading ||
        state.status != DomesticTripsItemsStatus.success) {
      return;
    }

    emit(state.copyWith(status: DomesticTripsItemsStatus.loadingMore));
    ...
    result.fold(
      (_) {
        emit(state.copyWith(status: DomesticTripsItemsStatus.success));
      },
      (data) {
        final response = DomesticTripsItemsResponse.fromJson(data);
        emit(
